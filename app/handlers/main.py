...
from ..db.subscriber_repository import SubscriberRepository
from ..models.subcriber_models import SubscriberRequest
...
def newsletter_subscribe(request: SubscriberRequest) -> Dict[str, str]:
    ...
    if not re.match(email_address_pattern, request.email_address):
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid email address: {request.email_address}"
        )
    ...
    if not subscriber.add_subscriber(request.email_address, request.user_name):
        ...
        raise HTTPException(
            status_code=409, detail=f"Unable to add subscriber: {request.email_address}"
        )
    else:
        return {
            "status": "success",
            "message": "Subscribed with {0}".format(request.email_address),
        }
...
def newsletter_subscribe(email_address: str):
    ...
    if subscriber.remove_subscriber(email_address):
        return {
            "status": "success",
            "message": "Feature in progress, we will remove {0} soon".format(
                email_address
            ),
        }
