...
from mypy.nodes import (ARG_NAMED, ARG_NAMED_OPT, ARG_OPT, ARG_POS, ARG_STAR,
                        ARG_STAR2, SYMBOL_FUNCBASE_TYPES, ArgKind, Context,
                        Decorator, Expression, MypyFile, ParamSpecExpr,
                        PlaceholderNode, SymbolTableNode, TypeAlias, TypeInfo,
                        TypeVarExpr, TypeVarLikeExpr, TypeVarTupleExpr, Var,
                        check_arg_kinds, check_arg_names, get_nongen_builtins)
from mypy.options import Options
from mypy.plugin import AnalyzeTypeContext, Plugin, TypeAnalyzerPluginInterface
from mypy.semanal_shared import (SemanticAnalyzerCoreInterface, paramspec_args,
                                 paramspec_kwargs)
from mypy.tvar_scope import TypeVarLikeScope
from mypy.types import (ANNOTATED_TYPE_NAMES, FINAL_TYPE_NAMES,
                        LITERAL_TYPE_NAMES, NEVER_NAMES, TYPE_ALIAS_NAMES,
                        AnyType, CallableArgument, CallableType, DeletedType,
                        ...
                        RawExpressionType, RequiredType, StarType,
                        SyntheticTypeVisitor, TrivialSyntheticTypeTranslator,
                        TupleType, Type, TypeAliasType, TypedDictType,
                        TypeList, TypeOfAny, TypeQuery, TypeType,
                        TypeVarLikeType, TypeVarTupleType, TypeVarType,
                        UnboundType, UninhabitedType, UnionType, UnpackType,
                        bad_type_type_item, callable_with_ellipsis,
...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def apply_concatenate_operator(self, t: UnboundType) -> Type:
        ...
        if not isinstance(ps, ParamSpecType):
            self.api.fail(
                "The last parameter to Concatenate needs to be a ParamSpec", t
            )
            return AnyType(TypeOfAny.from_error)
    ...
    def try_analyze_special_unbound_type(
        ...
        if fullname == "builtins.None":
            ...
            if not self.options.enable_incomplete_features:
                self.fail(
                    '"Unpack" is not supported yet, use --enable-incomplete-features', t
                )
                return AnyType(TypeOfAny.from_error)
    ...
    def analyze_type_with_type_info(
        ...
        if info.fullname == "types.NoneType":
            self.fail(
                "NoneType should not be used as a type, please use None instead", ctx
            )
    ...
    def analyze_literal_param(
        ...
        if isinstance(arg, AnyType):
            ...
            if arg.type_of_any not in (TypeOfAny.from_error, TypeOfAny.special_form):
                self.fail(
                    f'Parameter {idx} of Literal[...] cannot be of type "Any"', ctx
                )
            return None
...
def expand_type_alias(
    ...
    *,
    ...
    if act_len != exp_len:
        fail(
            f"Bad number of arguments for type alias, expected: {exp_len}, given: {act_len}",
            ctx,
        )
